 ...
 * */
export async function ifNullFAsync<A>(
  a: A,
  f: Supplier<Promise<A>>,
): Promise<A> {
...
export function caseFunctionLookup<A, B>(
  cases: Array<[A, () => B]>,
  target: A,
): B {
...
export function objForEach<A>(
  f: (a?: A, k?: ObjKey, o?: Obj<A>) => void,
): (o: Obj<A>) => void {
  return o => Object.keys(o).forEach(x => f(o[x], x, o));
}

export function objMap<A, B>(
  f: (a?: A, k?: ObjKey, o?: Obj<A>) => B,
): (o: Obj<A>) => B[] {
  return o => Object.keys(o).map(x => f(o[x], x, o));
}

export function objFilter<A>(
  f: (a?: A, k?: ObjKey, o?: Obj<A>) => boolean,
): (o: Obj<A>) => A[] {
  return o =>
    Object.keys(o)
      .filter(x => f(o[x], x, o))
...
export function tryWithDefault<A, B>(
  f: (...args: A[]) => B,
  defaultValue: B,
  args: A[] = [],
): B {
...
export function applyIf<A, B>(
  a: A | false | 0 | null | void,
  f: F1<A, B>,
): B | void {
  if (a) {
    return f(a);
...
/**@deprecated moved to noop.ts*/
export { gen_noop, noop } from './noop';
