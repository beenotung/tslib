...
import { scanRecursivelySync } from '../src/fs'
...
let file = 0
let total = 0
const format = (x: number) => Math.round(x * 100) / 100
const report = () => {
  const t = Date.now() - start
  process.stdout.write(
    `\r dir: ${dir} (${format(dir / t)}/s) file: ${file} (${format(
      file / t,
    )}/s) total: ${total} (${format(total / t)}/s)`,
  )
}
scanRecursivelySync({
