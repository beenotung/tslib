...
let url = 'http://127.0.0.1:3000/test/form/form-test.php';

async function main() {
  await test('postMultipartFormData',
    json => postMultipartFormData(url, json)
    // .then(res => res.data as string)
      .then(res => (is2xxResponse(res) ? Promise.resolve(res.data) : Promise.reject(res.data)) as Promise<string>),
  );

  await test('fetch',
    json => fetch(url, { method: 'POST', body: jsonToFormData(json) as any })
      .then(res => is2xxResponse(res)
        ? res.text()
        : res.text().then(reason => Promise.reject(reason))),
  );

  await test('checkedFetch',
    json => checkedFetch({
      input: url, init: { method: 'POST', body: jsonToFormData(json) as any },
      on2xx: res => res.text(),
